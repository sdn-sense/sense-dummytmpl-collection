...
def parse_commands(module, warnings):
    ...
    for _index, item in enumerate(commands):
        if module.check_mode and not item['command'].startswith('show'):
            warnings.append(f'only show commands are supported when using check mode, not executing `{item["command"]}`')
        elif item['command'].startswith('conf'):
            module.fail_json(msg='dummytmpl_command does not support running config mode commands.  Please use dummytmpl_config instead')
    return commands
