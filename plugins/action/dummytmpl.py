...
from ansible_collections.sense.dummytmpl.plugins.module_utils.network.dummytmpl import dummytmpl_provider_spec
...
class ActionModule(ActionNetworkModule):
    ...
    def run(self, tmp=None, task_vars=None):
        """dummytmpl Ansible Run"""

        self._config_module = self._task.action.split('.')[-1] == 'dummytmpl_config'
        sockPath = None
        ...
        if persConn == 'network_cli':
            ...
            provider = load_provider(dummytmpl_provider_spec, self._task.args)
            plc = copy.deepcopy(self._play_context)
            plc.connection = 'network_cli'
            plc.network_os = 'sense.dummytmpl.dummytmpl'
